...
data = pd.read_csv(r"C:\Users\Ahmed\Desktop\New folder\WSN-DS.csv")
...
for i, column in enumerate(data.columns[:-1], 1):  # Adjust grid size based on number of features
    plt.subplot(5, 4, i)
...
# Plot pair plots (on a reduced subset to avoid lag)
subset_features = data.columns[:3]  
subset_sample = data.sample(n=300, random_state=42)  
subset_data = subset_sample[subset_features.tolist() + [data.columns[-1]]]

unique_classes = subset_data[data.columns[-1]].nunique()
markers_list = ["o", "s", "D", "^", "+"][:unique_classes]

sns.set_context("notebook")  

sns.pairplot(subset_data, hue=subset_data.columns[-1], palette="tab10", markers=markers_list)
plt.suptitle("Pair Plots of Selected Features (Sampled Data)", y=1.02)
plt.show()
